...
func Test_decodeDeclineProgramVersionRequest(t *testing.T) {
	tests := []struct {
		name    string
	...
	}{
		{
			...
			wantErr: nil,
			want: declineprogramversion.NewCommand(
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Run(tt.name, func(t *testing.T) {
				got, err := decodeDeclineProgramVersionRequest(slog.Default())(context.TODO(), tt.request)
				if tt.wantErr == nil {
